...
LOG="$HOME/.olive/logs/ingestion.log"
...
Use the Bash tool to call `python3 ~/.olive/slack_api.py` for all Slack operations:
  ...
  - Send DM: echo "<message>" | python3 ~/.olive/slack_api.py send $SLACK_USER_ID
...
3. Search Slack for new messages:
   - Search 1: python3 ~/.olive/slack_api.py search "<@$SLACK_USER_ID>"
   - Search 2: python3 ~/.olive/slack_api.py search "from:$SLACK_USER_ID"
...
5. If items were extracted:
   ...
   c. Send a Slack DM: echo "I found [N] new item(s) for your review. Run \`olive approve\` to add them to your list. — Olive 🫒" | python3 ~/.olive/slack_api.py send $SLACK_USER_ID
